...
class VAEWithClassifier(VAE):
    def __init__(self, image_size, channel_num, kernel_num, z_size, num_classes):
        super().__init__(image_size, channel_num, kernel_num, z_size)
        self.classifier = nn.Linear(self.feature_volume, num_classes)

    def forward(self, x):
        # VAE forward pass
        (mean, logvar), x_reconstructed = super().forward(x)

        # Classification
        encoded = self.encoder(x).view(-1, self.feature_volume)
        class_logits = self.classifier(encoded)

        return (mean, logvar), x_reconstructed, class_logits
