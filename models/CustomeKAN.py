...
import numpy as np
from kan import KANLayer, Symbolic_KANLayer, LBFGS, curve2coef, SYMBOLIC_LIB
import sympy
import os
import glob
...
class KAN(nn.Module):
    ...
    def custom_train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0.,
                     ...
                     in_vars=None, out_vars=None, beta=3, save_fig_freq=1, img_folder='./video', device='cuda'):
        '''
        training
        ...
        def closure():
            ...
            if sglr_avoid is True:
                id_ = torch.where(torch.isnan(torch.sum(pred, dim=1)) is False)[0]
    ...
    def calculate_regularization(self, acts_scale, lamb_l1=1., lamb_entropy=2., lamb_coef=0.,
                                 lamb_coefdiff=0., small_mag_threshold=1e-16, small_reg_factor=1.):
